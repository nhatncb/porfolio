...
const useMutate = <TVariables = unknown, TError = unknown>(
  options: Options<TVariables, TError>,
) => {
  const { defaultToast, successMessage, tableName, ...otherOptions } = options;

  return useMutation({
    mutationFn: async (params: TVariables) => {
      const batch = writeBatch(db);
      const tableRef = doc(collection(db, tableName));
      batch.set(tableRef, params as { [x: string]: any });
      await batch.commit();
      return params;
    },
    onSuccess: () => {
